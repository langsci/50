...
The measures distinctiveness, parsimony, specificity and consistency were introduced by Edwin \citet{dejong:2000}. These measures are based on the entropy measure taken from information theory \citep{shannon:1948}. Entropy measures the uncertainty about a set of elements. The higher the entropy, the higher the uncertainty introducing chaos. Low entropy means order and less uncertainty. Information theory defines entropy as follows:\index{entropy}\index{De Jong, Edwin}
...
\begin{description}
\item[Discriminative success]\index{discriminative success} To monitor the ability of
...
\item[Parsimony]\index{parsimony} The parsimony $P_R$ is calculated similar to the distinctiveness:
\begin{eqnarray}
...
\item[Communicative Success]\index{communicative success} The communication success
...
\item[Actual success]\index{actual success} In order to say more about the
...
\item[Consistency]\index{consistency} Consistency measures how consistent a referent is named by a certain word-form. It is calculated as follows:
\begin{eqnarray}
...
\section{Sensory data}\label{s:st:data}\index{situation}
...
The experiment is a guessing game with feedback obtained by means of {\scshape correspondence} (see \sectref{s:cm:feedback}).\index{correspondence}
...
\index{prototype|(}
\index{ontology}
\todo{Table 1.3 should be aligned to join Sect. 1.3.1}
...
\label{f:st:semiotic}
\end{figure}\index{semiotic landscape}
